...
```{r}
## Loading the every required Libraries. 
...
dim(Universal_Bank)
t(t(names(Universal_Bank))) # The `t` function transposes the dataframe, swapping its rows and columns.

```
Removing the ID and ZIP Code
```{r}
## Removing unwanted items like ID and ZIP Code.
...
```{r}
## Initially, only the Education variable requires conversion to a factor type.

Universal_Bank$Education <- as.factor(Universal_Bank$Education)
levels(Universal_Bank$Education)
```
```{r}
## Next, transform the Education variable into dummy variables.
...
```
```{r}
## Dividing the dataset into training (60%) and validation (40%) subsets.
...
```{r}
Train_Norm_Different <- Train_Different[,-10] # Note that Personal Income is the 10th variable.
